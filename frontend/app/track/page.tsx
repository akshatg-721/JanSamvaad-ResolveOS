...
function slaCountdown(deadline?: string): { text: string; color: string } | null {
...
export default function TrackPage() {
  ...
  const handleFeedback = useCallback(async (stars: number) => {
    setRating(stars)
    if (!ticket?.id) return
  ...
  const sev = ticket ? (SEV[ticket.severity?.toUpperCase()] || SEV.MEDIUM) : SEV.MEDIUM
  ...
  return (
    <div className="min-h-screen bg-[#0A1628] text-white" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="border-b border-white/10 bg-[#071020] px-4 py-3">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <Link href="/" className="text-sm text-[#8A9BB5] hover:text-white">← Back to Home</Link>
          <Link href="/public" className="text-sm text-[#8A9BB5] hover:text-white">Public Data</Link>
        </div>
      </div>

      <div className="pt-10 pb-16 px-4 max-w-3xl mx-auto">
            ...
            <div className="rounded-xl border border-white/10 bg-[#112240] p-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                  ...
                  {isResolved ? '✅ RESOLVED' : '🔄 ' + String(ticket.status || 'OPEN').toUpperCase()}
                </span>
              </div>
