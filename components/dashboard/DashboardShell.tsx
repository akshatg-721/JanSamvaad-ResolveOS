...
import { useDashboardStats, useTickets, useRealtimeSocket, useActivity, type Ticket } from '@/hooks/use-dashboard';
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[11px] font-medium capitalize ${colors[normalized] ?? 'text-zinc-500'}`}>
...
function TicketTable({ statusFilter, severityFilter, search }: {
  ...
  const handleResolve = useCallback(async (ticketId: string) => {
    ...
    try {
      await updateComplaintStatus(ticketId, {
        status: 'RESOLVED',
  ...
  if (filtered.length === 0) {
    return (
      <div className="text-center py-24 text-[#8E8A80]">
        <p className="text-4xl mb-4">Inbox</p>
      ...
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-white/5">
          {filtered.map((ticket: Ticket) => {
            const isDone = ['resolved', 'closed'].includes(ticket.status.toLowerCase());
            return (
              <tr key={ticket.id} className="hover:bg-white/5 transition-colors duration-200 group">
                ...
                <td className="py-4 text-right">
                  <button
                    ...
                    disabled={isDone || resolvingIds[ticket.id]}
                  ...
                  >
                    {resolvingIds[ticket.id] ? '...' : isDone ? 'Done' : 'Resolve'}
                  </button>
                </td>
              </tr>
            );
...
function ActivityFeed() {
  ...
  const typeConfig: Record<string, { color: string; icon: string }> = {
    voice: { color: 'text-blue-400', icon: 'Voice' },
    escalation: { color: 'text-red-400', icon: 'Alert' },
    verify: { color: 'text-green-400', icon: 'Check' },
    system: { color: 'text-zinc-400', icon: 'System' },
  };
